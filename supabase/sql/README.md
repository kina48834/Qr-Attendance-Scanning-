...
| **Header profile card** (`AppLayout.tsx`) | `users.public_id` (random numeric user ID), `users.role`, `users.name`, `users.email`, `users.avatar` |
| **Profile pages (admin / organiser / teacher)** | `users.avatar` for uploaded profile photo + `users.password` updates (legacy accounts) / Supabase Auth password update for UUID users |
| **Student / teacher academic profile line** | `users.academic_*` + formatted `department`; run **`15_academic_enrollment_columns.sql`** on existing DBs (or full `07` / `00_all_in_one`) |
